...
async function filltables(data, table) {
    ...
    data.forEach((element) => {
        ...
        const downloadButtonHTML =
            element.FormStatus === "Closed"
                ? `<button class="download-btn info" data-id="${
                      element.id
        ...
        tablerow.innerHTML = `
            ...
            <td class="table-borders"><a href="details.html?id=${
                element.id
            }" tabindex="-1"><button class="info" tabindex="${tabindex++}">Details</button></a></td>
...
async function getData() {
    const response = await fetch(
        `http://localhost:5500/FilterSummaryInfo?supplierFilter=&status=Open&date1=0001-01-01&date2=9999-12-12`
    );
...
document
    ...
    .addEventListener("click", async function () {
        ...
        const response = await fetch(
            `http://localhost:5500/FilterSummaryInfo?supplierFilter=${supplier}&status=${status}&date1=${date1}&date2=${date2}`
        );
        const data = await response.json();
        const table = document.getElementById("table");
...
async function generatePDF(ncrId) {
    ...
    const ncrRes = await fetch(
        `http://localhost:5500/ncrFromID?ncrID=${ncrId}`
    );
    const qaRes = await fetch(
        `http://localhost:5500/QualityFromNCR?ncrID=${ncrId}`
    );
    const engRes = await fetch(
        `http://localhost:5500/EngineerFromNCR?ncrID=${ncrId}`
    );
    const purRes = await fetch(
        `http://localhost:5500/PurchasingFromNCR?ncrID=${ncrId}`
    );
    ...
    if (qa) {
        ...
        pdf.text(
            ...
            `Supplier/Receiving Inspection: ${
                qa.SupplierInspection ? "Yes" : "No"
            }`
        );
        pdf.text(
            20,
            80,
            `Work in Progress?: ${qa.WorkInProgress ? "Yes" : "No"}`
        );
        ...
        pdf.text(
            20,
            100,
            `Is Item Non-Conforming: ${qa.IsNonConforming ? "Yes" : "No"}`
        );
    ...
    if (eng) {
        ...
        pdf.text(
            100,
            145,
            `Notify Customer?: ${eng.NotifyCustomer ? "Yes" : "No"}`
        );
    ...
    if (pur) {
        ...
        pdf.text(
            100,
            240,
            `Follow Up Required: ${pur.FollowUpRequired ? "Yes" : "No"}`
        );
...
function checkEdit(ncrId, tabindex) {
    ...
    engRequest.send();
    const engJson =
        engRequest.status === 200 ? JSON.parse(engRequest.responseText) : null;

    const purRequest = new XMLHttpRequest();
    purRequest.open("GET", `http://localhost:5500/purchasing/${ncrId}`, false);
    purRequest.send();
    const purJson =
        purRequest.status === 200 ? JSON.parse(purRequest.responseText) : null;
